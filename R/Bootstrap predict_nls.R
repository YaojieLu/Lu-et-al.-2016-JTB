...
#' Also calculates a confidence interval using a case bootstrap resampling method 
#' (using \code{\link{bootCase}}) from the \code{car} package.
#' @param object An object of class 'nls'
#' @param from Plot line from this value (optional, if left alone, starts at minimum of data).
#' @param to Plot line to this value.
#' @param x Optional, a series of x values.
#' @param interval Either "none" (plot only the prediction line), or "confidence" in which case a confidence interval is plotted, based on a bootstrap resample.
#' @param level If interval="confidence", the confidence level (default 0.95).
#' @param n Number of points along the X axis to evaluate the curve (default = 101, as in \code{\link{curve}}).
#' @param nboot Number of bootstrap resamples to fit, using \code{\link{bootCase}}.
#' @param add If TRUE, adds the line to the current plot.
#' @param lwd Line width - a vector of length two (first element, line )
#' 
#'  add=FALSE, interval = c("none", "confidence"), lwd=c(1,1), lty=c(1,5), col=c("black", "red"), xlim=NULL, ylim=NULL, xlab=NULL, ylab=NULL,  ...){
#' @param \dots Further arguments, passed to \code{predict_nls} from \code{plot.nls}.
#' @export
#' @examples
#'\dontrun{
